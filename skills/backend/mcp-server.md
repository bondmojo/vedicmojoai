...
- `lib/mcpAuth.ts` — `resolveMcpUser(request)`: hashes `x-mcp-token` and resolves
  it to the owning `User.id` via a per-user `McpApiToken` row (generated by that
  user from `/account` — see `.kiro/specs/user-management/`); returns `null`
  (never an open bypass) on a missing/invalid/revoked token, except in
  non-production with `MCP_DEV_USER_EMAIL` set, which resolves a named dev
  user instead. Consumed through `lib/auth.ts`'s `resolveRequestUser`, which
  tries the browser session cookie first and falls back to this. Applied to
  every route the MCP calls, not just the two above — ownership mismatches
  404, matching the web UI's per-user chart scoping.
