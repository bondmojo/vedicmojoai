    ...
    >
      ...
      <div className="relative h-full w-full max-w-md bg-gray-950 border-l border-gray-700 flex flex-col overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700 shrink-0">
          <div>
            <h2 className="text-base font-semibold text-ink">Copy Chart for AI</h2>
            <p className="text-xs text-gray-500 mt-0.5">Select sections → Copy JSON → Paste into ChatGPT / Gemini</p>
          </div>
          <button
            onClick={onClose}
            className="rounded p-1.5 text-gray-400 hover:bg-gray-800 hover:text-ink transition-colors"
            aria-label="Close"
          >
            ✕
          </button>
                ...
                <div className="space-y-1">
                  {sections.map((s) => (
                    <label key={s.id} className="flex items-center gap-2.5 cursor-pointer group">
                      ...
                      />
                      <span className="text-sm text-gray-300 group-hover:text-ink transition-colors">{s.label}</span>
                    </label>
                  ))}
                </div>
              </div>
